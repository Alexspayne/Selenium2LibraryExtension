...
ROBOT_OPTIONS = [
    '--doc',
    'Selenium2Library acceptance tests with {browser}',
    '--outputdir',
    RESULTS_DIR,
    '--variable',
    'BROWSER:{browser}',
    ...
    'DEBUG',
    '--pythonpath',
    os.pathsep.join([SRC_DIR, TEST_LIBS_DIR]),
]
REBOT_OPTIONS = [
    '--outputdir',
    RESULTS_DIR,
...
def acceptance_tests(interpreter,
                     browser,
                     rf_options=[],
                     sauce_username=None,
                     sauce_key=None):
    ...
    with http_server():
        execute_tests(interpreter, browser, rf_options, sauce_username,
                      sauce_key)
...
def execute_tests(interpreter, browser, rf_options, sauce_username, sauce_key):
    ...
    options.extend([
        opt.format(browser=browser, py_version=interpreter + sys.version[:3])
...
def get_sauce_conf(browser, sauce_username, sauce_key):
    if browser == 'chrome' and TRAVIS:
        return []
    return [
        '--variable', 'SAUCE_USERNAME:{}'.format(sauce_username), '--variable',
        'SAUCE_ACCESS_KEY:{}'.format(sauce_key), '--variable',
        'REMOTE_URL:http://{}:{}@ondemand.saucelabs.com:80/wd/hub'.format(
            sauce_username, sauce_key), '--variable',
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--saucekey', '-K', help='Access key to order browser from SaucuLabs')
    ...
    if TRAVIS and browser != 'chrome' and TRAVIS_EVENT_TYPE != 'cron':
        print('Can not run test with browser "{}" from SauceLabs with PR.\n'
        ...
        sys.exit(0)
    sauce_username, sauce_key = sauce_credentials(args.sauceusername,
                                                  args.saucekey)
    # unit_tests()
    acceptance_tests(args.interpreter, browser, rf_options, sauce_username,
                     sauce_key)
